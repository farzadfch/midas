...
private[passes] class Fame1Transform(json: Option[java.io.File]) extends firrtl.passes.Pass {
    ...
    case s: Connect => s.loc match {
      case e: WSubField => ens get e.serialize match {
        case None => if (bb_clks.contains(e.serialize)) {
          ...
          // Create an instance of ClockBuf clock_buf (.I(clock), .CE(bb_targetFire), .O(bb_ap_clk));
          val clk_buf_ports = Seq(
             Field("I", Flip, UIntType(IntWidth(1))),
             Field("CE", Flip, UIntType(IntWidth(1))),
             Field("O", Default, UIntType(IntWidth(1)))
           )

          val clk_buf = WDefInstance(NoInfo, "clock_buf", "BUFGCE", BundleType(clk_buf_ports))
          ...
          Block(Seq(
            clk_buf,
            Connect(NoInfo, wsub(wref(clk_buf.name),"I"), castRhs(BoolType,s.expr)),
            Connect(NoInfo, wsub(wref(clk_buf.name),"CE"), targetFire),
            Connect(NoInfo, s.loc, castRhs( ClockType,wsub(wref(clk_buf.name),"O"))) // s.loc is the left side assigned signal
            )
          )
          } else s
        case Some(false) =>
  ...
  def run(c: Circuit) = {
    ...
    val clockBufPorts = Seq(
        Port(NoInfo,"I" ,Input,UIntType(IntWidth(1))),
        Port(NoInfo,"CE" ,Input,UIntType(IntWidth(1))),
        Port(NoInfo,"O" ,Output,UIntType(IntWidth(1)))
    )

    val clockBufModule = ExtModule(NoInfo, "BUFGCE", clockBufPorts, "BUFGCE", Seq())
    val c_modules = c.modules :+ clockBufModule
    //val c_modules = c_orig.modules
    c.copy(modules = c_modules map transform(moduleMap))
